...
//modifies values for each button click
function Input_Digit(digit) {
    const {Display_Value, Wait_Second_Operand} = Calculator;
    //checking to see if Wait_Second_Operand is true
    //and set Display_Value to key that was clicked
    if (Wait_Second_Operand == true) {
        Calculator.Display_Value = digit;
        Calculator.Wait_Second_Operand = false;
    }
    else {
        //overwrites display if current value is 0
...
function Input_Decimal(dot) {
    //ensures accidental clicking of decimal doesn't 
    //bug operation
    if (Calculator.Wait_Second_Operand === true) return;
    if (!Calculator.Display_Value.includes(dot)) {
        //adds decimal if there isn't one
...
function Handle_Operator(Next_Operator) {
    ...
    //when an operator is pressed, the current number is converted and 
...
keys.addEventListener('click', (event) => {
    const { target } = event;
    //if element that was clicked wasn't a button, exit function
